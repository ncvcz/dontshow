...
export default function Layout({ children, isSensitive }: LayoutProps) {
  ...
  // Show sensitive content alert if page is sensitive, alerts are enabled, and user hasn't chosen to continue
  if (isSensitive && settings.sensitiveAlert && !continueBrowsing) {
    return (
